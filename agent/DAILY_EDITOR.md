...
2. `docs/inbox/latest.json`을 읽습니다. 파일의 `day`가 당일 날짜와 다르면 `python3 -m blog_pipeline.collection.collect_news --today`를 한 번 실행합니다. 재실행 후에도 `day`가 당일과 다르거나 `selected`가 3건 미만이면 보존된 이전 `latest.json` 후보는 사용하지 않습니다. 당일 공식 발표·문서 2개와 독립 자료 1개 이상을 직접 검색해 교차 확인하거나, 충분한 자료가 없으면 초안 생성을 중단합니다. 정상적인 당일 후보함이면 `selected` 후보 5건을 먼저 보고, 적합한 주제가 없을 때만 `candidates`를 확인합니다.
   ...
   2. 모두 탈락하면 `candidates`에서 72시간 이내, 서로 다른 발행처의 상위 후보를 최대 12건까지 검토합니다. 제목·날짜·출처·최근 글 중복으로 먼저 좁히고, 가능성이 남은 후보만 원문을 엽니다.
   ...
   `BLOCKED`는 위 세 단계를 모두 수행해도 검증 가능한 후보가 없을 때만 허용합니다. 이때 보고에는 실제 검토한 후보 제목과 탈락 이유를 단계별로 남깁니다. `selected` 일부만 확인했거나 `candidates`에 미검토 최신 공식 후보가 남아 있으면 중단하지 않습니다.
...
- 제목은 `핵심 검색어 + 독자가 얻는 답이나 의외의 차이`로 만듭니다. 핵심 검색어는 한 번만 쓰고 앞 20자 안에 자연스럽게 배치합니다. 보통 35~60자 안에서 모바일 2~3줄을 목표로 하며 `충격`, `역대급`, `무조건 봐야` 같은 클릭베이트와 검색어 나열은 금지합니다.
