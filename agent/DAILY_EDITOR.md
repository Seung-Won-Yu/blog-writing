...
- 최근 7일 동안 적용 사례형 글이 없고, 당일 뉴스의 원리나 기능을 잘 보여 주는 성숙한 공개 저장소가 있을 때 우선 검토합니다. 주 1회 정도가 목표지만 횟수를 채우려고 무관한 저장소를 억지로 붙이지 않습니다.
- 유명함은 별 개수 하나로 판단하지 않습니다. 공식성, 최근 릴리스와 유지보수, 문서 품질, 실제 사용 흔적, 당일 주제와의 관련성을 함께 확인합니다.
- 공식 저장소, 관련 문서, 릴리스·PR·이슈, 실제 확인한 코드 경로를 자료에 포함합니다. 글에는 검증한 버전·커밋 또는 태그와 확인 날짜를 남겨 이후 변경과 구분합니다.
- 실행이 안전한 경우에만 별도 임시 디렉터리에서 최소 재현 예제를 돌립니다. 운영체제, 런타임, 의존성 버전, 실행 명령, 기대 결과와 실제 결과를 기록하고 핵심 기능 외의 전체 프로젝트를 불필요하게 설치하지 않습니다.
- 저장소 파일과 설치 명령을 먼저 읽습니다. 비밀키, 개인 계정, 운영 서비스, `sudo`, 알 수 없는 바이너리, 의심스러운 설치 스크립트·`postinstall`, 과도한 권한이 필요하면 실행하지 않습니다.
- 안전하게 실행하지 못했다면 코드·설정·공식 문서만 확인한 `구조 분석`으로 명시합니다. 이 경우 제목과 본문에서 실행·검증·벤치마크했다고 표현하지 않습니다.
- 글 흐름은 `문제 장면 → 이 저장소를 고른 이유 → 관련 구조와 코드 경로 → 재현 절차 → 적용 전후 → 실제 결과 → 한계와 적용 조건`을 사용합니다. 복사 가능한 최소 설정이나 코드를 넣되 저장소 원문을 길게 복제하지 않습니다.
- 이미지는 저장소의 실제 구성 요소·데이터 흐름, 적용 전후, 명령과 결과의 연결, 측정 결과를 설명합니다. 포괄적인 GitHub 로고·별 개수 장식·컴퓨터 화면은 쓰지 않으며, 측정하지 않은 숫자를 차트에 만들지 않습니다.
...
- 제목은 핵심 검색어와 독자의 실제 질문을 자연스럽게 담습니다.
...
- GitHub 적용 사례형은 저장소 이름을 빌린 소개 글이 아니라 재현 가능한 설정, 확인한 코드 경로, 실제 결과와 실패 조건이 중심이어야 합니다.
...
- GitHub 적용 사례라면 버전·커밋, 실행 환경, 명령, 실제 결과가 있고 실행하지 못한 내용은 구조 분석으로 구분했는가
...
- 관련 글 2개가 주제상 자연스럽고 실제 블로그 URL인가
