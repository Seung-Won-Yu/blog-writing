...
시작 시 `agent/BROWSER_QA.md`를 읽고 제작 전 검수 환경 확인과 누락 QA 재개 규칙을 우선 적용합니다.
...
2. `saturday_guard` 결과를 따릅니다.
   ...
   - `PARTIAL`: 출력된 `reasons`에 해당하는 단계만 복구합니다. 단, `quality_reader_access`는 중단 사유가 아니며 공통 독자 품질 루프로 원고 JSON을 자동 재편집합니다.
   - `NEW`: 아래 흐름을 정확히 한 번 수행합니다.

3. `python3 -m blog_pipeline.collection.inbox_guard --kind automation --today`를 먼저 실행합니다. `READY`면 후보함을 사용하고, `RECOLLECT_REQUIRED`면 보존된 이전 후보를 읽기 전에 `collect_automation --today`를 같은 실행에서 한 번만 다시 수행한 뒤 가드를 재실행합니다. 재실행 뒤에도 준비되지 않으면 이전 후보를 쓰지 않고 공식 저장소·공식 문서·원 논문 직접 리서치로 이동합니다. 그다음 `docs/automation-inbox/latest.json`을 읽습니다.
