...
검색형 제목은 독자가 얻게 될 결과를 제목 앞부분에 놓고 핵심 검색어를 자연스럽게 붙이며 도구명은 뒤에 둡니다. 예: `메일 첨부파일을 날짜별 폴더로 자동 정리하기: n8n 실험`. `충격`, `역대급`, `무조건` 같은 클릭베이트는 쓰지 않습니다. 태그 5~8개는 `자동화 대상`, `반복 문제`, `사용 도구`, `완성 결과`를 섞고 최소 2개를 `primary_query`와 직접 연결합니다. 전체는 약 10~15분 분량, 소제목 5~8개로 작성합니다.
