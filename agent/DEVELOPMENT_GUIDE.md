# 비활성 레거시 계약 — 수요일 14시 개발 가이드 실행 금지

이 문서는 과거 초안 ID와 테스트 호환을 위한 기록이며 활성 편집 계약이 아닙니다.
연결된 예약 작업은 `PAUSED` 상태로 유지하고 이 파일로 원고·이미지·커밋을 만들지
않습니다. 수요일 역할은 `agent/DAILY_EDITOR.md`의 09:00 `change_explainer`,
오래가는 개발 가이드는 같은 계약의 월요일 09:00 `evergreen_problem`이 담당합니다.
아래 내용은 역사 기록으로만 보존합니다. 예약 실행은 한 번만 수행하며 자동 재실행 슬롯을 두지 않습니다.
...
비교·Top N 주제는 기준일과 지표 정의를 먼저 고정하고 공식 API로 수치를 다시 확인합니다. 인기도와 품질을 같은 것처럼 쓰지 않고, 공통 평가 축·잘 맞는 상황·제약·업데이트 트리거를 함께 제시합니다. 여러 GitHub 저장소·공식 문서·Agent Skills를 엮은 지도·비교·커리어 분석은 금요일 개발·AI 인사이트로 보내고, 하나의 기술 원리·선택 기준은 월요일 개발 가이드에 통합합니다. 공식 발표가 예약 시각 기준 72시간 이내이고 독자의 당일 행동을 바꾸는 경우에만 수요일 IT 트렌드 해설의 도입 단서로 사용합니다.
...
티스토리에서는 `실전 IT > 개발 가이드`를 선택합니다. `실전 IT`에는 글을 직접 넣지 않습니다.
