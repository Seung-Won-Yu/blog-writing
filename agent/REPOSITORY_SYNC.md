...
`sh scripts/blog-env python3 ...`로 실행한다. 이 래퍼는 macOS에서 설치된
Homebrew Git/Python을 함께 우선해 Apple 기본 도구의 Xcode 약관 오류와 분리한다.
매 shell 호출에 래퍼를 적용하며 이전 shell의 PATH 변경이 유지된다고 가정하지 않는다.
처음에는 두 도구의 `--version`을 확인한다. 시스템 설정·Xcode 약관·사용자 shell
