---
title: "주식 선정 알고리즘: 급등주를 빼고 정배열을 보는 기준"
...
![급등 후보 카드가 황동 검사 게이트에서 멈추고 다른 후보가 구조 분석대로 넘어가는 주식 선정 작업대](../../docs/tistory/assets/2026-08-29-project/주식-선정-관문-대표.webp)

후보 200개를 모았지만, 어떤 순서로 탈락시킬지는 더 어려웠다. 가격이 싼
종목을 먼저 빼면 되는지, 거래대금과 스프레드 중 무엇을 앞에 둘지부터
애매했다. 당일 크게 오른 종목은 점수가 좋아 보였고, 정배열은 추세가 이어질
것처럼 보였다. 하지만 눈에 띄는 종목과 실제로 분석할 수 있는 종목은 같지
않았다.
...
> **후보 200개에서 급등주와 거래하기 어려운 종목을 어떤 기준으로 빼고,
> SMA20·SMA50 정배열은 어디에서 확인해야 할까?**

## 실제 구현은 한 줄 필터가 아니라 두 층이다

첫 층은 후보 공급 관문이다. 개별주 S1에 정밀 일봉과 호가 분석을 더 쓸
...
다시 확인한다. 이어 당일 +5% 초과 급등, ATR 범위,
`현재가>SMA20>SMA50` 구조를 본다.
...
여기서 예상 밖 결함이 하나 나왔다. 초기 코드는 등락률이 NaN이면 ‘+5%보다
큰가’를 확인하지 못한 채 관문을 건너뛰었다. 모르는 값이 ‘오르지 않은 값’처럼
취급된 셈이다. 2026-08-26 수정에서는 비유한 등락률을 큰 경고값으로 바꿔
급등 관문이 닫히도록 했고, 회귀 테스트로 다시 막았다. 이번 판단이 단순 채택이
아니라 **수정 후 채택**인 이유다.

![가격·호가·거래대금·급등률과 결측값 경계 테스트](../../docs/tistory/assets/2026-08-29-project/필터-경계-회귀테스트.webp)

*실행한 53개 집중 테스트 중 핵심 경계 사례를 재구성했다. 문턱 바로 바깥의
입력과 NaN 등락률은 통과하지 않는다.*
...
읽기 전용으로 확인한 모의투자 관찰 기록에는 S1이 설정 관문을 통과한 판단
152건 중 142건이 실제 진입으로 이어지지 않은 구간이 있다. 기록은 원인을
호가 깊이, 정수주, 현금 부족 같은 체결층 거부로 분리했다. 따라서 이 수치를
