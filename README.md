...
07:28 KST GitHub Actions 실행
           ...
           API 없이 1200×630 대표 이미지와 1200×675 본문 흐름 이미지 생성
...
대표 이미지와 본문 이미지는 생성형 AI 그림이 아니라, 그날 선정한 뉴스와 문구를 바탕으로 자동 조판하는 블로그 전용 정보 카드입니다. 그래서 매일 같은 인상을 유지하고 글자나 로고가 망가지는 문제가 적으며 추가 비용도 없습니다. 이미지 생성만 실패해도 뉴스 수집과 텍스트 초안은 그대로 남습니다.
