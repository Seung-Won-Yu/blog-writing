...
검증된 결과의 GitHub 전송은 `blog_pipeline.publishing.repository_sync`가 담당합니다. 활성 09:00 Codex 예약은 로컬 가드와 단일 커밋이 끝난 뒤 `origin/main` 한 곳만 이 명령으로 전송하도록 외부 쓰기 범위를 명시하며, 원시 push·강제 push·rebase·reset은 허용하지 않습니다. 토요일 묶음은 비공개 증거 유출 검사를 추가로 통과해야 합니다. DNS·연결 실패·timeout·HTTP 5xx만 짧은 지수 백오프로 최대 3회 재시도하고, 인증·권한·non-fast-forward는 즉시 중단해 강제 덮어쓰기를 막습니다. 최종 실패 시 로컬 커밋을 보존하므로 다음 예약 실행에서 이어서 전송할 수 있습니다.
