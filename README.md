...
- 매주 수요일 상시 검색형 `개발 가이드`를 뉴스와 분리해 복사·광고·미리보기 흐름으로 제공

## 운영 흐름

```text
05:17 KST · GitHub Actions
  → RSS·Atom·HTML 수집
  → URL·제목 정규화
  → 최근 사용 기사와 중복 확인
  → 출처·신선도·독자 관점 점수 계산
...
수요일 14:00 KST · Codex 개발 가이드 작업
  → 기존 글은 주제 출발점으로만 사용
  → 최신 공식 문서와 독립 자료 교차 확인
  → 10~20분 원고와 한국어 설명 이미지 3장 이상 제작
  → `실전 개발 노트 > 개발 가이드` 카테고리용 최종 HTML·미리보기 제공
  → 사용자가 수요일 18:00로 티스토리 예약 발행
```
...
- 실전 개발 노트: 월·목에 커뮤니티와 기술블로그에서 반복되는 문제 한 건을 골라 원리·사례·선택 기준까지 설명
- 개발 가이드: Java·Spring·PostgreSQL·API·보안·운영처럼 오래 검색되는 문제 해결 글
- 자동화·프로젝트 기록: 직접 실행한 자동화와 `pocket-desk-os` 같은 실제 프로젝트의 실패·수정·결과 기록

새 글은 세 축 중 하나에 분명히 속해야 하며, 같은 축의 공개 글 2개 이상과 연결합니다. 검색량만 기대한 낯선 주제를 넓게 다루거나 뉴스 원문을 바꿔 말하는 글은 만들지 않습니다. 제목은 핵심 검색어와 얻는 답을 함께 담고, 태그는 기술·문제·기능·사용 상황을 구체적으로 나타냅니다.

운영 판단은 하루 수익이 아니라 최근 7일과 이전 7일을 비교합니다.

- Search Console: 노출, 클릭, 검색어, 색인된 주요 글
- Tistory: 검색 유입 비중, 인기 글, 신규 글에서 이전 글로 이어지는 흐름
- AdFit: 요청, 유효 노출, 클릭률, eCPM, 예상 적립금

검색 노출은 늘지만 클릭이 없으면 제목을 점검하고, 클릭은 있지만 글 이동이 없으면 도입과 내부 링크를 점검합니다. 광고 eCPM만 떨어졌다면 광고를 추가하지 않고 최소 2주 추세를 봅니다.
