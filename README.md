...
Python 수집기
  ...
  → 결정적 Python 도구가 AdFit 포함 HTML·미리보기 생성
  → Git commit / push
...
4. Codex가 대표·본문 이미지를 만들고, Python 도구가 `NEWS 01` 뒤에 AdFit 마커가 든 티스토리 HTML·인라인 미리보기를 만듭니다.
...
2. `본문 미리보기`로 글과 이미지를 확인합니다.
3. 제목·태그와 `AdFit 포함 HTML 복사`를 선택합니다.
4. 티스토리 HTML 모드에 본문을 한 번 붙여넣습니다. AdFit은 `NEWS 01` 뒤에 이미 들어 있습니다.
5. 서식 손실을 막기 위해 기본 모드로 전환하지 않고, HTML 모드에서 저장합니다.
6. 대표 이미지·카테고리·공개 상태를 확인해 발행합니다.
