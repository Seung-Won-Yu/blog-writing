...
07:28 KST GitHub Actions 실행
           ...
           GitHub Models로 6~8분 분량의 사실·독자 영향·개발자 관점·확인할 점 생성
...
기본 모델은 무료 제한량이 넉넉한 `openai/gpt-4o-mini`입니다. 현재 GitHub Models의 Low 등급 무료 한도는 Copilot Free 기준 분당 15회, 하루 150회, 요청당 입력 8,000·출력 4,000토큰이며 이 자동화는 정상적으로는 하루 1회 호출합니다. 외부 기사와 히스토리가 길어져도 입력을 보수적으로 7,000토큰 이하로 줄여 여유를 남깁니다. 첫 응답이 지나치게 짧거나 상투적이거나 실제 6분 분량에 못 미치면 이전 JSON의 짧은 문단만 늘리는 품질 보정을 최대 두 번 요청합니다. 무료 한도는 GitHub 정책에 따라 바뀔 수 있습니다.
...
대표·본문 이미지는 글자를 가득 올린 PPT형 카드가 아니라, 기사의 핵심 데이터 흐름을 새로 그린 블로그 전용 장면형 일러스트입니다. 예를 들어 사진 AI 기사는 `휴대폰 사진 → AI 연동 → 사용자 통제`, OpenTelemetry 기사는 `개발 도구 → 관측 데이터 → 관리`로 표현합니다. 대표 이미지는 구체적인 대상과 짧은 질문만 남기고, 본문 이미지는 제목과 출처를 반복하지 않고 이해를 돕는 장면에 집중합니다. 추가 API 비용은 없으며 이미지 생성만 실패해도 뉴스 수집과 텍스트 초안은 그대로 남습니다.
