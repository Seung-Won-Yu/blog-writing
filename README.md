...
이미 발행했던 과거 글을 보강할 때는 날짜를 입력하고 `과거 후보함을 다시 수집하지 않고 현재 장문 구조로 보강`을 체크합니다. 이 모드는 해당 날짜의 `docs/inbox/YYYY-MM-DD.json`을 그대로 사용하므로 최신 뉴스가 과거 글에 섞이지 않습니다. 후보함이 없는 레거시 글은 기존 티스토리 HTML의 제목·요약·원문 링크를 먼저 복원합니다. 기존 HTML까지 없으면 실행을 중단하며, 보강 결과는 바로 발행하지 않고 복사 페이지에서 확인합니다.
