...
<body>
        ...
        <div class="content">
          ...
          <div class="builder">
            <section class="markup-box">
              ...
              <p>HTML 모드에서 방금 올린 이미지 줄 전체를 복사해 넣으세요. 반드시 티스토리가 만든 <code>[##_Image|..._##]</code> 태그를 사용하세요. 만료되는 <code>blog.kakaocdn.net?...expires=...</code> 주소는 차단합니다.</p>
  ...
  <script>
    ...
    function hasExpiringTistoryImageUrl(value) {
      const parsed = parseMarkup(value);
      return Array.from(parsed.querySelectorAll("img")).some((image) => {
        const src = String(image.getAttribute("src") || "");
        const isTistoryCdn = /(?:blog\.kakaocdn\.net|img\d*\.daumcdn\.net)/i.test(src);
        const hasSignedExpiry = /(?:[?&]|%26)(?:expires|signature)(?:=|%3D)/i.test(src);
        return isTistoryCdn && hasSignedExpiry;
      });
    }

    function extractImageMarkup(value) {
      const text = String(value || "");
      if (hasExpiringTistoryImageUrl(text)) return "";
      const parsed = parseMarkup(value);
    ...
    function isFinalHtmlStructurallyValid(value) {
      if (!value || value.includes(imageMarker) || value.includes(adMarker)
          || hasExpiringTistoryImageUrl(value)) return false;
      const parsed = parseMarkup(value);
